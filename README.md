# pooldepth — Aerodrome Slipstream pool depth, auto-refreshed to Dune
...
- `.github/workflows/refresh.yml` — daily 00:30 UTC run for ALIGN-USDC at 15-minute buckets
...
3. Actions tab → "Refresh ALIGN-USDC depth" → **Run workflow** to test.
4. Query it on Dune:

```sql
SELECT date_parse(hour, '%Y-%m-%dT%H:%i') AS ts, *
FROM dune.<your_handle>.dataset_align_usdc_depth_15m
ORDER BY 1
```
